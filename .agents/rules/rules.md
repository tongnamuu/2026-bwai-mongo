...
- 모든 생성 파일은 `hands-on/session-1/work/` 아래에 둡니다.
...
- 로컬 서버 설정은 `.env` 파일에서 `python-dotenv`로 읽습니다.
- 실제 비밀 값이 들어간 `.env` 파일을 만들거나 출력하지 않습니다.
...
- 명시적인 요청 전에는 긴 Ollama 생성 호출을 실행하지 않습니다.
